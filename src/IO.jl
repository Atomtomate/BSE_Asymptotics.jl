...
function setup(input_file::String, N_shell::Int)
    ...
    χ₀, χDMFTsp, χDMFTch, χ_sp_asympt, χ_ch_asympt, χ_pp_asympt, U, β, shift = jldopen(input_file, "r") do f
        χ₀ = calc_χ₀(gImp, f["β"], f["grid_nBose"], f["grid_nFermi"], f["grid_shift"])
        χ₀, permutedims(f["χDMFTsp"] ./ f["β"]^2, (2,3,1)), permutedims(f["χDMFTch"] ./ f["β"]^2, (2,3,1)), f["χ_sp_asympt"], f["χ_ch_asympt"], f["χ_pp_asympt"], f["U"], f["β"], f["grid_shift"]
    end
